...
const fallbackProjects = [
    {
        title: 'SimpleCDN',
        description: 'Saya membangun CDN yang ringan dan cepat ini dari nol untuk menyajikan file statis tanpa beban tambahan.',
        url: 'https://cdn.discordapp.my.id',
        image_url: 'https://cdn.discordapp.my.id/cdn/7541df.webp',
        full_width: 0,
        category: 'Edge delivery',
        chips: ['CDN', 'File statis', 'Caching']
    },
    {
        title: 'BerAPI',
        description: 'Kumpulan endpoint API yang saya buat untuk mempermudah prototyping cepat, integrasi AI, dan fitur unduh.',
    ...
    {
        title: 'Sistem Bot Otomatis',
        description: 'Bot WhatsApp yang saya tulis untuk mengelola komunitas, memoderasi obrolan, dan mengotomatiskan hal-hal yang membosankan.',
        url: null,
        image_url: 'https://cdn.discordapp.my.id/cdn/e3dece.webp',
        full_width: 0,
        category: 'Automation',
        chips: ['Bot', 'Moderasi', 'Support']
    }
];
...
const fallbackExperience = [
    {
        ...
        description: 'Mengelola infrastruktur backend utama, menjaga stabilitas game server saat beban tinggi, dan menyelesaikan masalah jaringan.',
    ...
    },
    {
        ...
        description: 'Membantu pelanggan memperbaiki masalah hosting, mengurus permintaan layanan, dan menjaga respons support tetap jelas.',
...
function renderProjects(projects) {
    ...
    projGrid.innerHTML = projects.map(p => {
        const title = p.title_id || p.title;
        const desc = p.description_id || p.description;
        const category = p.category || (p.full_width ? 'Cloud infrastructure' : 'Layanan digital');
        const chips = p.chips || ['Backend', 'Panel', 'Hosting'];
        const content = `
            <div class="proj-img">
                <img loading="lazy" src="${safeUrl(p.image_url)}"${p.image_url_fallback ? ` data-fallback="${safeUrl(p.image_url_fallback)}"` : ''} alt="${escHtml(title)}" onerror="if(this.dataset.fallback&&this.src!==this.dataset.fallback){this.src=this.dataset.fallback}else{this.closest('.proj-img')?.classList.add('image-timeout');this.removeAttribute('src')}">
            </div>
            <div class="proj-content">
                <div class="proj-topline"><span class="proj-icon"><i class="fa-solid fa-server"></i></span><span>${escHtml(category)}</span></div>
                <h3 class="proj-title">${escHtml(title)}</h3>
                <p class="proj-desc">${escHtml(desc)}</p>
                <div class="proj-tags">${chips.map(chip => `<span>${escHtml(chip)}</span>`).join('')}</div>
                <span class="proj-link-hint">${p.url ? 'Buka proyek ↗' : 'Build privat'}</span>
            </div>`;
        const classes = `project-card ${p.full_width ? 'full-width' : ''} ${p.url ? 'is-clickable' : ''}`;
        return p.url ? `<a href="${safeUrl(p.url)}" target="_blank" rel="noopener noreferrer" class="${classes}" aria-label="Buka proyek ${escHtml(title)}">${content}</a>`
...
function renderExperience(experiences) {
    ...
    expTimeline.innerHTML = experiences.map(e => {
        const role = e.role_id || e.role;
        const desc = e.description_id || e.description;
        ...
        <${tag}${href} class="${classes}">
            ...
            <div class="exp-info">
                ...
                <div class="exp-role">${escHtml(role)}</div>
                <div class="exp-date">${escHtml(e.date_range)}</div>
                <p class="exp-desc">${escHtml(desc)}</p>
            </div>
        </${tag}>
    `}).join('');
...
async function loadComments() {
    ...
    commentList.innerHTML = '<div class="empty-note">Memuat komentar...</div>';
    try {
        const res = await fetch(`${API_BASE}/api/comments`);
        const comments = await res.json();
        if (!Array.isArray(comments) || !comments.length) {
            commentList.innerHTML = '<div class="empty-note">Belum ada komentar. Jadilah yang pertama.</div>';
            return;
        }
        ...
        commentList.innerHTML = `<div class="empty-note error">${local ? 'Jalankan backend di port 3001 untuk memuat komentar.' : 'Komentar gagal dimuat.'}</div>`;
    }
}
...
document.querySelectorAll('[data-auth-mode]').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        authSubmit.textContent = mode === 'register' ? 'Buat akun' : 'Login';
...
authForm?.addEventListener('submit', async event => {
    ...
    try {
        ...
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Permintaan akun gagal.');
        commentUser = json.user;
        authForm.reset();
        renderCommentUser();
        setStatus(mode === 'register' ? 'Akun siap. Komentar bisa langsung tampil.' : 'Berhasil login.');
    } catch (err) {
...
commentForm?.addEventListener('submit', async event => {
    ...
    try {
        ...
        const json = await res.json();
        if (!res.ok || !json.ok) throw new Error(json.error || 'Komentar gagal dikirim.');
        commentForm.reset();
        if (window.turnstile) turnstile.reset();
        setStatus(json.message || 'Komentar terkirim.', json.status === 'pending' ? '' : 'success');
        await loadComments();
    } catch (err) {
        setStatus(err.message || 'Komentar gagal dikirim.', 'error');
    } finally {
